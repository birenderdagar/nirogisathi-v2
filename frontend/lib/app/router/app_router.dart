...
import '../../features/home/presentation/pages/subscription_detail_page.dart';
import '../../features/home/presentation/pages/plan_your_day_page.dart';
import '../../features/home/domain/entities/planner_event.dart';
...
import '../../features/health_locker/domain/entities/health_locker_items.dart';
...
import '../../features/home/domain/entities/user_subscription.dart';
...
final GoRouter router = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      path: '/plan-your-day',
      builder: (context, state) {
        final editEvent = state.extra is PlannerEvent ? state.extra as PlannerEvent : null;
        return PlanYourDayPage(editEvent: editEvent);
      },
    ),
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        if (extra is SubscriptionPaymentExtra) {
          return PaymentGatewayScreen(
            totalAmount: extra.amount,
            subscriptionPayment: extra,
          );
        }
    ...
    GoRoute(
      path: '/health-locker/prescription-detail',
      builder: (context, state) {
        final item = state.extra as PrescriptionRecord?;
        if (item == null) return const PrescriptionsScreen();
        return PrescriptionDetailScreen(item: item);
      },
    ),
    GoRoute(
      path: '/health-locker/add-prescription',
      builder: (context, state) => AddPrescriptionScreen(
        existing: state.extra is PrescriptionRecord ? state.extra as PrescriptionRecord : null,
      ),
    ),
    
    GoRoute(path: '/health-locker/lab-reports', builder: (context, state) => const LabReportsScreen()),
    GoRoute(
      path: '/health-locker/lab-report-detail',
      builder: (context, state) {
        final item = state.extra as LabReportRecord?;
        if (item == null) return const LabReportsScreen();
        return LabReportDetailScreen(item: item);
      },
    ),
    GoRoute(
      path: '/health-locker/add-lab-report',
      builder: (context, state) => AddLabReportScreen(
        existing: state.extra is LabReportRecord ? state.extra as LabReportRecord : null,
      ),
    ),
    
    GoRoute(path: '/health-locker/diagnostics', builder: (context, state) => const DiagnosticsScreen()),
    GoRoute(
      path: '/health-locker/diagnostic-detail',
      builder: (context, state) {
        final item = state.extra as DiagnosticRecord?;
        if (item == null) return const DiagnosticsScreen();
        return DiagnosticDetailScreen(item: item);
      },
    ),
    GoRoute(
      path: '/health-locker/add-diagnostic',
      builder: (context, state) => AddDiagnosticScreen(
        existing: state.extra is DiagnosticRecord ? state.extra as DiagnosticRecord : null,
      ),
    ),

    GoRoute(path: '/health-locker/medicines', builder: (context, state) => const MedicinesScreen()),
    GoRoute(
      path: '/health-locker/medicine-detail',
      builder: (context, state) {
        final item = state.extra as MedicineRecord?;
        if (item == null) return const MedicinesScreen();
        return MedicineDetailScreen(item: item);
      },
    ),
    GoRoute(
      path: '/health-locker/add-medicine',
      builder: (context, state) => AddMedicineScreen(
        existing: state.extra is MedicineRecord ? state.extra as MedicineRecord : null,
      ),
    ),
  ],
);
