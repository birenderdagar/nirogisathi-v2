...
class _MyCalendarState extends State<MyCalendar> {
  ...
  Widget build(BuildContext context) {
    return Consumer<DayPlannerProvider>(
          ...
          child: Column(
            ...
            children: [
              ...
              if (provider.eventsForSelectedDay.isEmpty)
                ...
                _buildScrollableTaskList(provider.eventsForSelectedDay),
              const SizedBox(height: 10),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  onPressed: () => _showAllDayEvents(context, provider),
  ...
  Widget _buildScrollableTaskList(List<PlannerEvent> events) {
    // Fixed viewport for 2 items (avoids bottom overflow).
    const double listHeight = 148;
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) => _buildPlannerItem(events[index]),
      ),
    );
  }
                      ...
                      if (hasEvents)
                        Positioned(
                          ...
                          child: Container(
                            ...
                            height: 5,
                            decoration: BoxDecoration(
                              color: isSelected ? Colors.white : const Color(0xFFFBB03B),
                              shape: BoxShape.circle,
                            ),
                          ),
                        ),
                  ...
                  children: [
                    ...
                    if (event.reminderEnabled)
                      const Padding(
                        ...
                        child: Icon(Icons.alarm, size: 14, color: Color(0xFF2196F3)),
                      ),
                  ],
                ),
                Text(
          ...
          Column(
            ...
            children: [
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  if (value == 'edit') {
                    context.push('/plan-your-day', extra: event);
  ...
  Future<void> _confirmDelete(PlannerEvent event) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete activity?'),
        content: Text('Remove "${event.title}" from your day planner?'),
        actions: [
    ...
    if (confirmed == true && mounted) {
      await context.read<DayPlannerProvider>().removeEvent(event.id);
    }
  }
            ...
            return DraggableScrollableSheet(
              ...
              builder: (context, scrollController) => Column(
                children: [
                  ...
                  Text(
                    'Activities for ${DateFormat('dd MMM yyyy').format(liveProvider.selectedDate)}',
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                  ),
                  const Divider(),
                  Expanded(
                    child: liveProvider.eventsForSelectedDay.isEmpty
                        ? const Center(child: Text('No events planned for this day'))
                        : ListView.separated(
                            padding: const EdgeInsets.all(20),
                            controller: scrollController,
                            itemCount: liveProvider.eventsForSelectedDay.length,
                            separatorBuilder: (_, _) => const Divider(),
                            itemBuilder: (context, index) =>
                                _buildPlannerItem(liveProvider.eventsForSelectedDay[index]),
                          ),
                  ),
                ],
              ),
