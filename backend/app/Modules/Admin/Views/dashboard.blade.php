...
<aside class="w-72 bg-[#071220] text-white p-6 flex flex-col shadow-2xl">
    ...
    <div class="flex items-center gap-3 mb-10">
        <img src="{{ asset('images/logo.png') }}" class="w-12 h-12 rounded-xl" alt="Logo">
        <div>
...
<div class="bg-white rounded-2xl p-5 flex justify-between items-center shadow-sm mb-8">
    ...
    <div>
        ...
        <p class="text-slate-500">
            {{ $adminPost ?? 'Administrator' }} ·
            <span id="currentDateTime">
                {{ now()->format('l, d M Y • h:i A') }}
            </span>
        </p>
...
<div class="bg-white rounded-2xl p-6 shadow-sm border">
    <p class="text-slate-500">{{ $card[0] }}</p>
    <h3 class="text-3xl font-bold">{{ $card[1] }}</h3>
    <span class="text-green-500">{{ $card[2] }}</span>
</div>
...
new Chart(document.getElementById('visitChart'), {
    ...
    data: {
        labels: ['Male','Female'],
        datasets: [{ data: {!! json_encode($genderData) !!} }]
    }
});
</script>
